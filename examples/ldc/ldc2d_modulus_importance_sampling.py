...
def train(cfg: DictConfig):
    ...
    v = sp.Function("v")(x, y)
    changes_mag = (
        u.diff(x) ** 2 + u.diff(y) ** 2 + v.diff(x) ** 2 + v.diff(y) ** 2
    ) ** 0.5 + 10
    importance_measure = ppsci.utils.symbolic.lambdify(
        changes_mag,
        model,
        create_graph=False,
    )
    ...
    bc_top = ppsci.constraint.BoundaryConstraint(
        ...
        {
            "dataset": {
                "name": "ImportanceSamplingIterableNamedArrayDataset",
    ...
    bc_noslip = ppsci.constraint.BoundaryConstraint(
        ...
        geom["rect"],
        {
            "dataset": {
                "name": "ImportanceSamplingIterableNamedArrayDataset",
                "batch_size": cfg.TRAIN.batch_size.bc_noslip,
            },
            "iters_per_epoch": cfg.TRAIN.iters_per_epoch,
            "batch_size": cfg.TRAIN.batch_size.bc_noslip,
        },
    ...
    pde = ppsci.constraint.InteriorConstraint(
        ...
        geom["rect"],
        {
            "dataset": {
                "name": "ImportanceSamplingIterableNamedArrayDataset",
                "batch_size": cfg.TRAIN.batch_size.pde,
            },
            "iters_per_epoch": cfg.TRAIN.iters_per_epoch,
            "batch_size": cfg.TRAIN.batch_size.pde,
        },
...
@hydra.main(
    version_base=None,
    config_path="./conf",
    config_name="ldc2d_modulus_importance_sampling.yaml",
)
