...
class RotaryEmbedding(nn.Layer):
    def __init__(
        self,
        dim,
        custom_freqs: (paddle.Tensor | None) = None,
        ...
        self.register_buffer(
            name="cached_freqs",
            tensor=paddle.zeros(cache_max_seq_len, dim),
            persistable=False,
        )
        self.cached_freqs_seq_len = 0
        self.freqs = nn.Parameter(freqs, requires_grad=learned_freq)
        self.learned_freq = learned_freq
        self.register_buffer(name="dummy", tensor=paddle.tensor(0), persistable=False)
        ...
        self.register_buffer(name="scale", tensor=scale, persistable=False)
        self.register_buffer(
            name="cached_scales",
            tensor=paddle.zeros(cache_max_seq_len, dim),
            persistable=False,
    ...
    def rotate_queries_and_keys(self, q, k, seq_dim=None):
        ...
        device, dtype, seq_len = q.place, q.dtype, q.shape[seq_dim]
    ...
    def get_axial_freqs(
        self, *dims, offsets: (tuple[int | float, ...] | paddle.Tensor | None) = None
    ):
        Colon = slice(None)
        all_freqs = []
        if exists(offsets):
